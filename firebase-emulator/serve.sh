...
export firebase_port_storage=${FIREBASE_PORT_STORAGE:-9199}
...
export firebase_port_auth_proxy=$(( $firebase_port_auth + 101 ))
...
(
    firebase emulators:start 2>&1 \
        ...
        --only auth${database_support:+,database},storage \
            | sed -ur 's/^/:: [firebase] /'
) &
firebase_pid=$!
...
echo ":: waiting for firebase to start up - storage"
:wait:port $firebase_port_storage
echo ":: ok"
